...
public class Aimlock {
    ...
    private static DoState doState = DoState.NOTE;
    ...
    private SimpleMotorFeedforward aimFF = new SimpleMotorFeedforward(0.0, 19);
    private ProfiledPIDController aimPID = new ProfiledPIDController(5, 0.02, 0.5,
            new Constraints(Math.toRadians(180), Math.toRadians(180)));

    private SimpleMotorFeedforward RRaimFF = new SimpleMotorFeedforward(0.0, 3.75);
    private ProfiledPIDController RRaimPID = new ProfiledPIDController(1.5, 0.0, 0.045,
            new Constraints(Math.toRadians(180), Math.toRadians(180)));
    ...
    public double getAngleToSpeaker() {
        double toSpeakerAngle = Math.toDegrees(
                Math.atan((5.55 - m_swerve.getRoboPose2d().getY()) / (m_swerve.getRoboPose2d().getX() - 0.3)));
    ...
    public double getShooterTargetAngleSPEAKER() {
        ...
        // vertical vector
        double Vy = m_shooter.getDesiredShooterSpeed() * Math.sin(getVerticalAngleToSpeaker()); // change
                                                                                                // m_shooter.getDesiredShooterSpeed()
                                                                                                // to getShooterMPS()
                                                                                                // later
        // horizontal vector

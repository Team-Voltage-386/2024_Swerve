...
public class RobotContainer {
  ...
  // private final CommandXboxController m_manipController = new
  // CommandXboxController(Controller.kManipController);
  ...
  public RobotContainer() {
    // pass le aimbot. once we convert all this to command based we will pass the
    // aim to the command instead of the subsystem. make it work for now, then we
    // will make it right!
    ...
    // NamedCommands.registerCommand("Pickup Piece", new autoPickupNote(m_swerve));
  ...
  private void configureBindings() {
    ...
    m_driveController.rightTrigger(0.25).onTrue(this.m_swerve.toggleFieldRelativeCommand());

    // manip cont bindings
    // m_manipController.x().onTrue(Commands.runOnce(() ->
    // Aimlock.setDoState(DoState.NOTE)));
    // m_manipController.a().onTrue(Commands.runOnce(() ->
    // Aimlock.setDoState(DoState.SPEAKER)));
    // m_manipController.b().onTrue(Commands.runOnce(() ->
    // Aimlock.setDoState(DoState.AMP)));
    // m_manipController.y().onTrue(Commands.runOnce(() ->
    // Aimlock.setDoState(DoState.SOURCE)));
    // new Trigger(() -> m_cameraSubsystem.isLLOdoGood(5.0))
    // .whileTrue(m_rumble.setRumbleCommand(RumbleType.kBothRumble, 0.5, 0.25));
  }
  ...
  private void configPathPlannerStuff() {
    ...
    // );
    pathfindAmp = AutoBuilder.pathfindToPose(new Pose2d(1.81, 7.68, Rotation2d.fromDegrees(90)), constraints);
  }
  ...
  public Command getAutonomousCommand() {
    return path1;
    // return new autoPickupNote(m_swerve);
  }
